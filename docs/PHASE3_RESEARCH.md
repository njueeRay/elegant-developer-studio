...
- 全站搜索：应放到 Phase 4。
- reactions：应放到 Phase 5。
- CMS/admin：应放到 Phase 5。
...
- Web lightbox 模式：灯箱必须支持键盘关闭、上一张/下一张和移动端安全布局。
...
- `photos` 的信息密度：精选大图优先，还是 masonry grid 优先？
...
1. `/photos`：精选照片 + 网格 + 灯箱。
2. `/music`：当前 mix + track list + mini player。
...
- 当前曲目信息清晰。
- 可以保留轻微程序员语义，例如 track id、mix context、状态。
